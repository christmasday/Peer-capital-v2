...
export default function OnrampRequestsPage() {
  const [requests, setRequests] = useState<OnrampRequest[]>([])
  const [loading, setLoading] = useState(true)
  const [refreshing, setRefreshing] = useState(false)
  const [search, setSearch] = useState("")
  const [statusFilter, setStatusFilter] = useState("all")
  const [selectedRequest, setSelectedRequest] = useState<OnrampRequest | null>(null)
  ...
  const fetchRequests = useCallback(async (showRefreshing = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/onramp-requests?${params}`, {
        credentials: 'include'
      })
      ...
      if (response.ok) {
        ...
        toast({
          title: "Error",
          description: "Failed to fetch onramp requests",
          variant: "destructive",
        })
      }
    } catch (error) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'payment_confirmed':
        return <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100"><Wallet className="w-3 h-3 mr-1" />Payment Confirmed</Badge>
      case 'failed':
  ...
  const getEventTypeBadge = (eventType: string) => {
    switch (eventType) {
      ...
      case 'wallet.funding.completed':
        return <Badge variant="outline" className="text-xs">Wallet Funded</Badge>
      default:
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Onramp Requests</h1>
          <p className="text-muted-foreground">
            View and monitor all user onramp (fiat to crypto) requests
          </p>
        </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <form onSubmit={handleSearch} className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="Search by request ID, user ID, or transaction ref..."
                  value={search}
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Payment Confirmed</CardTitle>
            <Wallet className="h-4 w-4 text-blue-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">
              {requests.filter(r => r.status === 'payment_confirmed').length}
            </div>
          </CardContent>
        </Card>
        <Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Recent Onramp Requests</CardTitle>
          <CardDescription>
            A list of all onramp requests from webhook events
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
                ...
                <TableBody>
                  {requests.map((request) => (
                    <TableRow key={request.id}>
                      ...
                      <TableCell>
                        <Badge variant="secondary">{request.network}</Badge>
                      </TableCell>
      ...
      <Dialog open={!!selectedRequest} onOpenChange={() => setSelectedRequest(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Onramp Request Details</DialogTitle>
            <DialogDescription>
              Full details for this onramp request
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedRequest && (
            <div className="space-y-6">
              ...
              {/* Sender Details */}
              {(selectedRequest.senderName || selectedRequest.accountNumber) && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {selectedRequest.senderName && (
                      <div>
                        ...
                        <p className="mt-1">{selectedRequest.senderName}</p>
                      </div>
                    )}
